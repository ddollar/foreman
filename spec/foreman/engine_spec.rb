...
describe "Foreman::Engine" do
  ...
  describe "start" do
    it "forks the processes" do
      ...
      mock(subject).watch_for_termination.with({}, {})
      subject.start
    end
    ...
    it "handles concurrency" do
      ...
      mock(subject).watch_for_termination.with({:concurrency => 'alpha=2'}, {})
  ...
  describe "environment" do
    ...
    it "should read if specified" do
      mock(subject).watch_for_termination({:env => "/tmp/env"}, {'FOO' => 'baz'})
      File.open("/tmp/env", "w") { |f| f.puts("FOO=baz") }
      subject.execute("alpha", :env => "/tmp/env")
    end

    it "should fail if specified and doesnt exist" do
      mock(subject).watch_for_termination({:env => "/tmp/env"}, {})
      mock(subject).error("No such file: /tmp/env")
      subject.execute("alpha", :env => "/tmp/env")
    end

    it "should read .env if none specified" do
      mock(subject).watch_for_termination({}, {'FOO' => 'qoo'})
